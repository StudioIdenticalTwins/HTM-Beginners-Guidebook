...
予測したミニカラムのアクティブセルの抽出は、pythonの `set()` 関数と `sorted()` 関数を使用しています。

時間プーリングはSDRを複数重ね合わせた形状をしています。各SDRアプティブセルを和集合（ユニオン）することで1枚のSDRに変換します。

SDRを抽出するにはアクティブセルを numpy 配列に変換し、 `sdr.sparse` をつかって代入します。
